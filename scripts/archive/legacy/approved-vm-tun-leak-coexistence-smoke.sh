...
usage() {
  cat >&2 <<'EOF'
usage: MAVERICK_TUN_LEAK_APPROVED=1 scripts/approved-vm-tun-leak-coexistence-smoke.sh <ssh-host>

Runs a namespace-scoped TUN leak/coexistence smoke on an explicitly approved
Linux VM over SSH. It must never be pointed at the developer's local machine.

What it mutates on the remote host:
  - creates a temporary network namespace;
  - creates a temporary veth pair for control-plane coexistence checks;
  - creates a temporary TUN device inside the namespace;
  - adds namespace-local preserved control-plane and default routes;
  - writes a namespace-scoped resolv.conf;
  - compares host listener/default-route/global-DNS baselines before and after;
  - rolls all temporary state back and verifies no residue remains.
...
host="${1:-${MAVERICK_TUN_LEAK_HOST:-}}"
approved="${MAVERICK_TUN_LEAK_APPROVED:-}"
namespace="${MAVERICK_TUN_LEAK_NETNS:-mavleak$$}"
tun_device="${MAVERICK_TUN_LEAK_TUN:-mavtl$$}"
veth_host="${MAVERICK_TUN_LEAK_VETH_HOST:-mavlh$$}"
veth_ns="${MAVERICK_TUN_LEAK_VETH_NS:-mavln$$}"
echo_port="${MAVERICK_TUN_LEAK_ECHO_PORT:-24553}"
...
if [[ "$approved" != "1" ]]; then
  echo "MAVERICK_TUN_LEAK_APPROVED=1 is required" >&2
  usage
  exit 2
fi
...
case "$host" in
  localhost|127.0.0.1|::1)
    echo "refusing to run approved VM TUN leak/coexistence smoke against local host: $host" >&2
    exit 2
    ;;
esac
...
echo "==> run approved VM TUN leak/coexistence smoke on $host"
...
TUN_ADDR="10.255.3.1/30"
VETH_HOST_ADDR="198.18.2.1/30"
VETH_NS_ADDR="198.18.2.2/30"
VETH_HOST_IP="198.18.2.1"
CONTROL_PLANE_CIDR="${VETH_HOST_IP}/32"
DNS_SERVER="9.9.9.9"
ECHO_LOG="/tmp/maverick-tun-leak-${NETNS}-echo.log"
ECHO_PID=""
...
snapshot_host_listeners() {
  ss -H -lntu 2>/dev/null | awk '{print $1, $5}' | sort || true
}
...
if [[ "$(uname -s)" != "Linux" ]]; then
  echo "approved VM TUN leak/coexistence smoke requires Linux" >&2
  exit 1
fi
...
listeners_before="$(snapshot_host_listeners)"
...
echo "apply: veth control-plane coexistence path"
...
echo "namespace_control_plane_uses_veth=ok"

echo "apply: create TUN and default route inside namespace"
...
sudo -n ip netns exec "$NETNS" ip route add default dev "$TUN_DEVICE" metric 4273

for probe in 1.1.1.1 8.8.8.8 93.184.216.34; do
  sudo -n ip netns exec "$NETNS" ip route get "$probe" | grep -q "dev $TUN_DEVICE"
done
echo "namespace_default_probe_uses_tun=ok"

sudo -n ip netns exec "$NETNS" ip route get "$DNS_SERVER" | grep -q "dev $TUN_DEVICE"
echo "namespace_dns_probe_uses_tun=ok"

echo "verify: control-plane TCP echo still works while default route points to TUN"
rm -f "$ECHO_LOG"
...
data = conn.recv(4096)
conn.sendall(b"maverick-leak-ok:" + data)
conn.close()
sock.close()
PY
ECHO_PID="$!"
...
with socket.create_connection((host, port), timeout=5) as sock:
    ...
    data = sock.recv(4096)
if data != b"maverick-leak-ok:probe":
    raise SystemExit(f"unexpected echo response: {data!r}")
print("namespace_coexistence_echo=ok")
PY
kill "$ECHO_PID" 2>/dev/null || true
wait "$ECHO_PID" 2>/dev/null || true
ECHO_PID=""
...
listeners_after="$(snapshot_host_listeners)"
...
if [[ "$listeners_before" != "$listeners_after" ]]; then
  echo "host listener baseline changed unexpectedly" >&2
  diff -u <(printf '%s\n' "$listeners_before") <(printf '%s\n' "$listeners_after") || true
  exit 1
fi

echo "host_listeners_unchanged: true"
echo "default_route_unchanged: true"
echo "global_dns_unchanged: true"
echo "approved_vm_tun_leak_coexistence_smoke=ok"
REMOTE
